...
def CreateCourses(connection):
    ...
    session_ids = session_model.search([('attendee_ids', '!=', False)])
    res = session_model.export_data(session_ids , ['id', 'name', 'course_id/name', 'course_id/responsible_id', 'attendee_ids/id', 'attendee_ids/name'])
    for rec in res['datas']:
        print rec
